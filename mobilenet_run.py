...
model = get_mobilenet_v2(5).to(device)
...
for epoch in range(epochs):
    ...
    print(f"[MobileNetV2] Epoch {epoch+1}/{epochs} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.4f}")

torch.save(model.state_dict(), "mobilenet_lc25000.pth")
